...
class Table:
    ...
    def _select_where(self, return_columns, condition=None, order_by=None, desc=True, top_k=None, distinct=False):
        ...
        Args:
            ...
            distinct: bool. If True the list of row will constist only unique rows (non duplicate rows). False by default.
        '''
        ...
        if condition is not None:
            ...
            if "like" in condition.split():
                ...
                #converting reg --> python
                regex = re.compile(conditions[2].replace("%",".*").replace("_",".").replace("'",""))
                rows = [index for index, x in enumerate(column) if regex.fullmatch(str(x))]
            #if between is in command
            elif "between" in condition.split():
                conditions = condition.split()#conditions list
                down = conditions[2]
                up = conditions[4] #the upper value
                column_name = conditions[0]
                column = self.column_by_name(column_name)
                try:
                    rows = [index for index, x in enumerate(column) if x <= up and x>=down]
                except:
                    rows = [index for index, x in enumerate(column) if x<= int(up) and x >= int(down)]
            #if in is in the command
            elif "in" in condition.split():
                conditions = condition.split()
                val = conditions[conditions.index('(')+1:conditions.index(')')]
                val = [va.replace(',','') for va in val] #re-creating val
                conditions.remove('(')
                conditions.remove(')')


                if len(conditions) == 3:
                    val = conditions[2].split(',')
                else:
                    val = []
                    for va in conditions[2:]:
                        v = va.split(',')
                        for _ in v:
                            if _ != '':
                                val.append(_)
                column_name = conditions[0]
                column = self.column_by_name(column_name)
                rows =[index for index, x in enumerate(column) if (str(x) in val)]
            else:
                column_name, operator, value = self._parse_condition(condition)
                column = self.column_by_name(column_name)
                rows = [index for index, x in enumerate(column) if get_op(operator,x,val)]
        else:
